...
def parse(path):
    ...
    with open(path) as f:
        for line in f:
            ...
            c, t = int(left), float(right)
            chunks.append(c)
            times.append(t)
    return chunks, times


def min_segment(chunks, times, pct=0.01, smooth_window=15):
    arr = np.array(times)
    half = smooth_window // 2
    smoothed = np.array(
        [np.median(arr[max(0, i - half) : i + half + 1]) for i in range(len(arr))]
    )
    mn = smoothed.min()
    threshold = mn * (1 + pct)
    ...
    while hi < len(smoothed) - 1 and smoothed[hi + 1] < threshold:
        hi += 1
    return chunks[lo], chunks[hi], min(times), min(times) * (1 + pct)


parser = argparse.ArgumentParser(description="Plot execution time vs chunk size")
...
parser.add_argument(
    ...
    type=float,
    default=1.0,
    help="threshold percentage above the minimum (default: 1.0)",
)
parser.add_argument(
    "--smooth", type=int, default=15, help="median smoothing window (default: 15)"
)
...
for i, file_spec in enumerate(args.files):
    ...
    chunks, times = parse(path)

    ax.plot(chunks, times, color=color, linewidth=0.9, alpha=0.85, label=label)

    x0, x1, mn, threshold = min_segment(
        chunks, times, pct=pct, smooth_window=args.smooth
    )
    ...
    ax.scatter(chunks[min_idx], times[min_idx], color=color, s=80, zorder=5)
    deferred_min.append((chunks[min_idx], times[min_idx], color, i))
...
ax_top.set_xticklabels(
    [f"{int(xb):,}" for xb, _ in band_boundaries], rotation=40, ha="left", fontsize=7
)
...
ax.set_title("Execution time vs chunk size", color="white", fontsize=13, pad=12)
ax.set_xlabel("Chunk size (bytes)", color="#aaaaaa", fontsize=10)
ax.set_ylabel("Time (ms)", color="#aaaaaa", fontsize=10)
ax_r.set_ylabel("Threshold (ms)", color="#666666", fontsize=9)
