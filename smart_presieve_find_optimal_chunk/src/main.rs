...
const CHUNK_FROM: usize = 32000;
const CHUNK_TO: usize = 256000;
const CHUNK_STEP: usize = 1000;
...
fn sieve(n: usize, chunk_size: usize) -> Vec<u8> {
    ...
    for from in (0..sieve_len).step_by(chunk_size) {
        let to = (from + chunk_size - 1).min(sieve_len - 1);
...
fn main() -> io::Result<()> {
    let mut x = 0;
    for chunk_size in (CHUNK_FROM..=CHUNK_TO).step_by(CHUNK_STEP) {
        let mut m = Measure::new();
        for _ in 0..BENCHMARKS {
            let t = Instant::now();
            let sieve = sieve(N, chunk_size);
            m.add(t.elapsed());

            x ^= sieve[chunk_size % N];
        }

        let avg = m.avg();
        println!("{chunk_size} - {avg:.1}");
    }
